...
class OfficialKnockoutResultsScreen
    extends StatefulWidget {

  final String roomId;

  const OfficialKnockoutResultsScreen({
    super.key,
    required this.roomId,
  });

  @override
  State<OfficialKnockoutResultsScreen>
      createState() =>
          _OfficialKnockoutResultsScreenState();
}

class _OfficialKnockoutResultsScreenState
    extends State<OfficialKnockoutResultsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Official Knockout Results')),
...
Widget buildKnockoutMatchCard(
  ...
  Future<void> loadOfficialBracket()
async {
...
final savedKnockout =
  ...
  restoreRound(
    ...
    List<Map<String,dynamic>>.from(
      savedData['round_of_32'] ?? [],
    ),
  );

  final round16Saved =
      List<Map<String,dynamic>>.from(
    savedData['round_of_16'] ?? [],
  );

  if (round16Saved.isNotEmpty) {
  ...
  final quarterSaved =
      List<Map<String,dynamic>>.from(
    savedData['quarter_finals'] ?? [],
  );

  if (quarterSaved.isNotEmpty) {
  ...
  final semiSaved =
      List<Map<String,dynamic>>.from(
    savedData['semi_finals'] ?? [],
  );

  if (semiSaved.isNotEmpty) {
